...
const CourseCard = ({ duration, title, imgSrc }) => (
  <div className="border rounded-lg p-4 relative min-w-[200px] sm:min-w-0">
    ...
    <img
      ...
      className="mx-auto mb-4 h-auto object-cover"
      width="40"
      height="40"
    />
    <h2 className="text-center font-semibold mb-2">{title}</h2>
    <button className="bg-blue-600 text-white px-4 py-2 rounded-full mx-auto block">
      View
    </button>
  </div>
);
  ...
  useEffect(() => {
    const updateVisibleCourses = () => {
      if (window.innerWidth >= 1024) {
        setVisibleCourses(courses.length); // Show all courses on large screens
      } else {
        setVisibleCourses(6); // Limit courses on smaller screens
      }
    };
  ...
  return (
    <div className="bg-white text-gray-800 py-[2%]">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex mb-8 bg-white overflow-x-auto scrollbar-hide">
        ...
        <div className="text-center mt-8">
          <a className="text-blue-600 cursor-pointer" href="#">
