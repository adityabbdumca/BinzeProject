  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 md:hidden flex justify-between items-center bg-white pt-4 pb-4 px-6
      ...
      {navItems.map((item, index) => (
        <button
          key={index}
          className={`relative flex flex-col items-center focus:outline-none group transition-all duration-300 
        ...
        >
          ...
          <span className={`text-[10px] font-semibold tracking-wide transition-all duration-300 
                            ${item.isActive ? 'text-blue-600 translate-y-0' : 'text-gray-600 group-hover:text-gray-800 translate-y-1'}`}>
            {item.label}
          </span>
